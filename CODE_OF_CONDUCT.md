# Code of Conduct

## Our Pledge

DicomGhost is a security research project. Everyone who participates — contributors, users, and maintainers — is expected to engage respectfully and professionally.

## Standards

Acceptable behaviour:
- Constructive technical discussion
- Respectful disagreement
- Helping newcomers learn
- Giving and receiving feedback graciously

Unacceptable behaviour:
- Harassment of any kind
- Sharing real patient data or PHI in issues, PRs, or discussions
- Using DicomGhost or this community to facilitate unauthorized access to healthcare systems
- Deliberate spread of misinformation about healthcare security

## Enforcement

Violations can be reported to the maintainer via the email in SECURITY.md. All reports will be handled confidentially. Maintainers reserve the right to remove comments, close issues, or ban users who violate this policy.

## Scope

This code of conduct applies in all project spaces — GitHub issues, pull requests, discussions, and any other community forum associated with this project.
